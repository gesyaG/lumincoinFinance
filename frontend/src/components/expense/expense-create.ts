    ...
    private initBtnEvents(): void {
        if (this.createBtn) {
            this.createBtn.addEventListener('click', async () => {
                ...
                const errorEl: HTMLElement | null = document.getElementById('newExpenseCategoryError');
                ...
                const response: CategoryType = await ExpenseService.createExpenseCategory(createData as DataCategoryType);
