...
export class IncomeCategories {
        ...
        if (this.deleteConfirmBtn) {
            this.deleteConfirmBtn.addEventListener('click', async () => {
                if (this.currentDeleteId) {
                    const result: CategoryType = await IncomeService.deleteIncomeCategory(this.currentDeleteId);
    ...
    private async getIncomeCategories(): Promise<void> {
        const response: ReturnCategoriesObjectType = await IncomeService.getIncomeCategories();
